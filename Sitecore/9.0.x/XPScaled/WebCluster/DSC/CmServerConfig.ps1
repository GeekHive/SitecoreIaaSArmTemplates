Configuration CmServerConfig
{
    ...
    {	
        ...
        Script installXconnectCert {
			...
			TestScript = {
                $cert = Get-ChildItem -Path Cert: -Recurse | Where-Object subject -match "sitecore"
				[boolean]$cert
			}
        }
        
        Script exportXconnectCert {
			GetScript = { @{Result = ($true)}}
			SetScript = {
                $thumbprint = (Get-ChildItem -Path Cert:\LocalMachine\My | Where-Object {$_.Subject -match "_client"}).Thumbprint;
                $pwd = ConvertTo-SecureString -String "$using:solrSSLPassword" -Force -AsPlainText
                $path = 'cert:\localMachine\My\' + $thumbprint
                Export-PfxCertificate -cert $path -FilePath "C:\xConnectClient.pfx" -Password $pwd -ChainOption BuildChain
			}
			TestScript = {
                $false
            }
            DependsOn  = "[Script]installXconnectCert"
		}
